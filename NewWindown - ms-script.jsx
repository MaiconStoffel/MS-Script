(function () {
  ...
  function TelaInicial() {
    ...
    function criarBotao(grupo, texto, simbolo, onClick, helpTip) {
      var botao = grupo.add("button", undefined, simbolo + " " + texto);
    ...
    criarBotao(coluna1, "Troca de Aspas", "💻", function () {
      telaAspas();
    });

    criarBotao(coluna1, "Capitalização", "#", function () {
      telaCapitalizacaoTitulos();
    });

    criarBotao(coluna2, "Separador decimal", "@", function () {
      telaDecimais();
    }, "Troca do separador decimal (ponto/vírgula)" );

    criarBotao(coluna2, "Ajustes gerais", "+", function () {
      telaEspacos();
    });

    var dev = janela.add("statictext", undefined, "Developed by: @mshcwertz - Version 2.4");

    try { dev.graphics.font = ScriptUI.newFont("Times New Roman", "italic", 5);
    } catch(e) {}
  ...
  function telaDecimais() {
    ...
    // Checkbox: aplicar em todas as páginas
    var chkTodasPaginas = win.add(
      "checkbox",
      undefined,
    ...
    // Campo de páginas (desabilitado por padrão)
    win.add("statictext", undefined, "Páginas a afetar (ex: 1-3,5,8):");
    ...
    proximo.onClick = function () {
      ...
      if (!aplicarEmTodasAsPaginas) {
        ...
        if (paginasArray.length === 0) {
          alert("Nenhuma página válida foi detectada. Verifique o formato.");
          return;
        }
  ...
  function telaEspacos() {
    ...
    var chkPontoFinalEspaco = win.add(
      "checkbox",
      undefined,
      "Padronizar espaço após ponto final '. '"
    );
    var chkVirgulaEspaco = win.add(
      "checkbox",
      undefined,
      "Padronizar espaço após vírgula ', '"
    );
    var chkIgualEspaco = win.add(
      "checkbox",
      undefined,
      "Padronizar espaço antes e depois do sinal ' = '"
    );
    var chkEspacoAntesPercent = win.add(
      "checkbox",
      undefined,
      "Padronizar espaço antes do símbolo ' %'"
    );
    var chkDoisPontosEspaco = win.add(
      "checkbox",
      undefined,
      "Padronizar espaçamento dos dois-pontos ':'"
    );
  ...
  function telaCapitalizacaoTitulos() {
    ...
    win.add(
      "statictext",
      undefined,
      "Escolha o idioma e os estilos de parágrafo para aplicar a capitalização:"
    );
    ...
    var chkNaoAplicar = win.add(
      "checkbox",
      undefined,
      "Não aplicar capitalização de títulos"
    );
    ...
    proximo.onClick = function () {
      ...
      if (
        ...
        alert(
          "Selecione ao menos um estilo ou marque 'Não aplicar capitalização de títulos'."
        );
        return;
      }
  ...
  function telaFinal() {
    ...
    if (aplicarAspas)
      resumo += "- Correção das aspas para o idioma: " + idiomaEscolhido + "\n";
    if (trocarPontoPorVirgula)
      resumo += "- Trocar ponto por vírgula decimal (3.14 → 3,14)\n";
    if (trocarVirgulaPorPonto)
      resumo += "- Trocar vírgula por ponto decimal (3,14 → 3.14)\n";
    if (ajustarPontoFinalEspaco)
      resumo += "- Padronizar espaço após ponto final '. '\n";
    if (ajustarVirgulaEspaco)
      resumo += "- Padronizar espaço após vírgula ', '\n";
    if (ajustarIgualEspaco)
      resumo += "- Padronizar espaço antes e depois do sinal ' = '\n";
    if (ajustarEspacoAntesPercent)
      resumo += "- Padronizar espaço antes do símbolo ' %'\n";
    if (ajustarEspacamentoDoisPontos)
      resumo += "- Padronizar espaçamento do dois-pontos ':'\n";
    if (ajustarEspacamentoPontoVirgula)
      resumo += "- Padronizar espaçamento do ponto e vírgula ';'\n";
  ...
  function executarCorrecao() {
    ...
    if (ajustarVirgulaEspaco)
      mensagem +=
        "✅ " + contadorVirgulaEspaco + " - Espaços após vírgula ajustados.\n";
    if (ajustarIgualEspaco)
      mensagem +=
        "✅ " +
        contadorIgualEspaco +
        " - Espaços em torno do sinal '=' ajustados.\n";
    if (ajustarEspacamentoDoisPontos)
      mensagem +=
        "✅ " +
        contadorEspacamentoDoisPontos +
        " - Espaçamentos dos dois-pontos ajustados.\n";
    if (ajustarPontoFinalEspaco)
      mensagem +=
        "✅ " +
        contadorPontoFinalEspaco +
        " - Espaços após ponto final ajustados.\n";
    if (ajustarEspacoAntesPercent)
      mensagem +=
        "✅ " +
        contadorEspacoAntesPercent +
        " - Espaços antes do '%' ajustados.\n";
    if (ajustarEspacamentoPontoVirgula)
      mensagem +=
        "✅ " +
        contadorEspacoDepoisPontoVirgula +
        " - Espaçamentos do ';' ajustados. \n";
...
//FUNCOES DE EXECUCAO, FAZEM AS ALTERACOES NO ARQUIVO UTILIZANDO GREP
    ...
    try {
        ...
        for (var i = 0; i < partes.length; i++) {
            ...
            if (parte.indexOf("-") > -1) {
                ...
                if (isNaN(inicio) || isNaN(fim) || fim < inicio || inicio < 1 || fim > totalPaginas) {
                ...
                if (!isNaN(num) && num >= 1 && num <= totalPaginas && !arrayContem(resultado, num)) {
                resultado.push(num);
                } else {
    ...
    function removerEspacoAntesVirgula() {  //remove qualquer espaço antes de uma virgula
    app.findGrepPreferences = app.changeGrepPreferences = null;
    ...
    function corrigirEspacoDepoisVirgula() {    //corrige a "," depois de uma LETRA, nao funciona de for numeros
    app.findGrepPreferences = app.changeGrepPreferences = null;
