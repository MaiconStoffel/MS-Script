(function () {
    ...
    var chkTodasPaginas = win.add("checkbox", undefined, "Aplicar a todas as páginas");
    ...
    if (!aplicarEmTodasAsPaginas) {
        paginasSelecionadasDecimal = inputPaginas.text ? trim(inputPaginas.text) : "";

        if (!paginasSelecionadasDecimal || paginasSelecionadasDecimal.length === 0) {
            alert("Por favor, digite a(s) página(s) a ser(em) alterada(s) ou marque 'Aplicar a todas as páginas'.");
            return;
        }
        ...
        if (paginasArray.length === 0) {
            alert("Verifique se você preencheu corretamente as páginas que deseja fazer a mudança");
            return;
        }
    } else {
