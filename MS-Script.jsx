...
(function () {  
  ...
  function GerarCodigo_2D ()  {
    if (app.documents.length === 0) {
      alert("Abra e salve um documento antes de executar.");
      return;
    }
    var doc = app.activeDocument;
    ...
    function waitFile(path, timeoutMs) {
      var f = new File(path),
        waited = 0,
        step = 200;
      while (!f.exists && waited < timeoutMs) {
        $.sleep(step);
        waited += step;
      }
      return f.exists;
    ...
    function runZintViaVB(zintPath, dataText, outPath, scale, logPath) {
      ...
      var vb =
        ...
        "\r\n" +
        'Dim f: Set f = fso.CreateTextFile("' +
        vbEsc(logPath) +
        '", True)' +
        "\r\n" +
        'f.WriteLine "Cmd=' +
    ...
    function gerarEColocarDM(numeroDoc, alvoPx, escala) {
      ...
      if (!zint) {
        alert(
          "Zint não configurado. Rode o Set-ZintPath.jsx e selecione o zint.exe."
        );
        return;
      }
      ...
      var outPath =
        outDir.fsName + "/" + conteudo.replace(/[^0-9A-Za-z_-]/g, "_") + ".png";
      ...
      // Info final
      var msg =
        "Data Matrix gerado com sucesso!\n\n" +
        "Conteúdo: " +
        conteudo +
        "\n" +
        "Arquivo:  " +
        outPath +
        (useTemp
          ? "  (TEMP – mova para Links se quiser manter o vínculo)"
          : "");
      alert(msg);
    }
    ...
    function testarZint() {
      var zint = getZintPath();
      if (!zint) {
        alert("ZINT_EXE_PATH não definido.");
        return;
      }
      ...
      var f = new File(outTxt),
        waited = 0;
      while (!f.exists && waited < 4000) {
        $.sleep(200);
        waited += 200;
      }
      var ver = "";
    ...
    var g1 = w.add("group");
    g1.add("statictext", undefined, "Número do documento (só dígitos):");
    var edt = g1.add("edittext", undefined, "");
    edt.characters = 20;

    var g2 = w.add("group");
    g2.add("statictext", undefined, "Tamanho alvo (px):");
    var edtSize = g2.add("edittext", undefined, "30");
    edtSize.characters = 6;

    var g3 = w.add("group");
    g3.add("statictext", undefined, "Escala (px por módulo):");
    var edtScale = g3.add("edittext", undefined, "6");
    edtScale.characters = 6;
    ...
    ok.onClick = function () {
      ...
      if (!/^\d+$/.test(n)) {
        alert("Informe apenas dígitos do número do documento.");
        return;
      }
